...
fn main() {
    ...
    let model = std::path::PathBuf::from(home)
        .join("Library/Application Support/Parle/models/ggml-base-q5_1.bin");
    ...
    let l2 = levels.clone();
    let rec = parle_audio::recorder::Recorder::start("", move |u| {
    ...
    if parle_asr::is_silence(&recording.samples) {
        println!("SILENCE");
        return;
    }
