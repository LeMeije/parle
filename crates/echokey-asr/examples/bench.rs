...
//!   cargo run --release --example bench -p echokey-asr --features metal   (macOS)
//!   cargo run --release --example bench -p echokey-asr --features cuda    (Windows)
...
fn default_models_dir() -> std::path::PathBuf {
    #[cfg(target_os = "macos")]
    {
        std::path::PathBuf::from(std::env::var("HOME").unwrap())
            .join("Library/Application Support/EchoKey/models")
    }
    #[cfg(target_os = "windows")]
    {
        std::path::PathBuf::from(std::env::var("LOCALAPPDATA").unwrap()).join("EchoKey/models")
    }
    #[cfg(not(any(target_os = "macos", target_os = "windows")))]
    {
        std::path::PathBuf::from(std::env::var("HOME").unwrap()).join(".local/share/EchoKey/models")
    }
}
