//! LAN discovery over mDNS: advertise `_parle._tcp` and browse for peers.
//!
...
pub const SERVICE_TYPE: &str = "_parle._tcp.local.";
...
/// well-formed Parle advertisement.
...
/// stable and cannot collide with another Parle install.
fn host_name(id: &DeviceId) -> String {
    format!("parle-{}.local.", id.short())
}
...
mod tests {
    ...
    fn instance_and_host_names_are_legal_labels() {
        ...
        assert_eq!(host_name(&config.device_id), "parle-3f2b1c4d.local.");
    }
    ...
    fn r4_the_fullname_map_is_unbounded_and_lan_controlled() {
        ...
        assert!(
            ...
            "precondition: every synthesised announcement must be a well-formed \
             Parle record that translate actually accepts, but only {found} of \
             {RECORDS} were"
        );
